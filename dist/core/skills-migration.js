...
import { logSkillTarget, physicalProjectPath, resolveSkillTargets } from './skill-targets.js';
...
async function readMode(file) {
    try {
        ...
        if (!stat.isFile() || stat.isSymbolicLink())
            throw new Error(`Unsafe managed file: ${file}`);
...
async function matchesFile(file, bytes, mode) {
...
async function rejectLinkedSkillEntry(projectDir, relative) {
...
async function inventory(directory) {
    try {
        ...
        if (!stat.isDirectory() || stat.isSymbolicLink())
            throw new Error(`Unsafe managed skill directory: ${directory}`);
    }
    catch (error) {
        if (error.code === 'ENOENT')
            return null;
    ...
    const result = { files: new Map(), modes: new Map(), directories: [''] };
    async function visit(current, prefix) {
        for (const entry of await fs.readdir(current, { withFileTypes: true })) {
            const relative = prefix ? `${prefix}/${entry.name}` : entry.name;
            const file = path.join(current, entry.name);
            if (entry.isSymbolicLink())
                throw new Error(`Preserving linked entry; migration requires manual resolution: ${file}`);
            if (entry.isDirectory()) {
                ...
                throw new Error(`Unsupported managed skill entry: ${file}`);
        }
    }
...
function managedHash(files, raw = false) {
...
export async function collectSkillOwners(installedExtensions) {
...
async function renderComposition(owner, runtime, context, extensions) {
    ...
    for (const { dir, manifest } of extensions) {
        for (const injection of manifest.injections ?? []) {
            if (injection.target !== owner.name)
                continue;
            const bytes = await readOptional(path.join(dir, injection.file));
            if (!bytes)
                throw new Error(`Missing injection source for ${manifest.name}:${owner.name}`);
...
export async function preflightSkillMigration(projectDir, config, groups) {
    ...
    if (extensions.length !== (config.extensions ?? []).length)
        throw new Error('Missing installed extension manifest; resolve it before skill migration.');
    ...
    const warnings = [];
    const provenRoots = new Map();
    async function addFile(relative, before, after, beforeMode, afterMode) {
    ...
    for (const group of resolved) {
        const participants = group.targets.map(target => config.agents.find(agent => agent.id === target.id)).filter((agent) => !!agent);
        ...
        for (const agent of participants)
            provenRoots.set(agent.skillsDir, await physicalProjectPath(projectDir, agent.skillsDir));
        ...
        for (const target of group.targets) {
            if (target.sourcePhysicalPath === group.physicalPath)
                continue;
            ...
            for (const entry of entries)
                if (!names.has(entry))
                    warnings.push(`Preserving unmanaged entry: ${target.previousSkillsDir}/${entry}`);
        }
        logSkillTarget('preflight:start', { target: group.skillsDir, skills: [...names] });
        for (const name of names) {
            if (!name || name === '.' || name === '..' || /[/\\]/.test(name))
                throw new Error(`Unsafe installed skill name: ${name}`);
            const owner = owners.get(name);
            if (!owner)
                throw new Error(`Unknown source for "${name}"; preserve the existing copy and resolve its ownership before migration.`);
            ...
            for (const agent of participants) {
                ...
                if (proofs.some(proof => proof.physical === physical))
                    continue;
                ...
                if (!equalFiles(tree.files, expected))
                    throw new Error(`Skill migration conflict: "${directory}" differs from its known source (including injections or unknown files). Preserve both copies and resolve local changes.`);
                if (!owner.extension) {
                    const state = (await buildManagedSkillsState(projectDir, agent, [name], oldContext))[name];
                    ...
                    if (!record || record.version !== manifest?.version)
                        throw new Error(`Unproven extension revision for "${directory}".`);
                }
                proofs.push({ directory, physical, tree, files: expected });
            }
            if (proofs.length === 0)
                throw new Error(`No installed baseline for "${name}"; restore the source before migrating to ${group.skillsDir}.`);
            ...
            for (const [relative, bytes] of finalFiles) {
                const modes = new Set([...proofs.map(proof => proof.tree), ...(destinationTree ? [destinationTree] : [])]
                    .map(tree => tree.modes.get(relative)).filter((mode) => mode !== undefined));
                if (modes.size !== 1)
                    throw new Error(`Skill migration mode conflict: "${name}/${relative}" has differing or unknown permissions. Preserve copies and resolve their modes.`);
            ...
            for (const [relative, bytes] of destinationTree?.files ?? []) {
                if (!finalFiles.has(relative))
                    await addFile(`${destination}/${relative}`, bytes, null, destinationTree.modes.get(relative), null);
            }
            for (const proof of proofs) {
                if (proof.physical === destinationPath)
                    continue;
                for (const [relative, bytes] of proof.tree.files)
                    await addFile(`${proof.directory}/${relative}`, bytes, null, proof.tree.modes.get(relative), null);
                ...
                for (const relative of proof.tree.directories) {
                    if (knownDirectories.has(relative))
                        cleanupDirectories.add(relative ? `${proof.directory}/${relative}` : proof.directory);
                    else
                        warnings.push(`Preserving unknown directory: ${proof.directory}/${relative}`);
                }
            }
            ...
            for (const agent of next.agents.filter(agent => participants.some(participant => participant.id === agent.id))) {
                agent.managedSkills ??= {};
                if (owner.extension)
                    delete agent.managedSkills[name]; // A replacement must not retain the old bundled-skill receipt.
                if (sourceState && agent.installedSkills.includes(name)) {
                    const state = { sourceHash: sourceState.sourceHash, installedHash: managedHash(finalFiles),
        ...
        for (const agent of next.agents) {
            if (participants.some(participant => participant.id === agent.id))
                agent.skillsDir = group.skillsDir;
        }
    }
    if (!await matchesFile(path.join(projectDir, '.ai-factory.json'), configBefore, configBeforeMode))
        throw new Error('Config revision or permissions changed during migration preflight. Retry after resolving concurrent edits.');
...
const bytesDigest = (bytes) => bytes === null ? null : createHash('sha256').update(bytes).digest('hex');
...
async function checkedRuntimeRoot(projectDir) {
    const root = await physicalProjectPath(projectDir, MIGRATION_ROOT);
    await fs.mkdir(root, { recursive: true, mode: 0o700 });
    if ((await fs.lstat(root)).isSymbolicLink())
        throw new Error('Migration state directory must not be a link.');
    return root;
}
export async function withSkillProjectLock(projectDir, action) {
    ...
    for (let attempt = 0;; attempt++) {
        try {
            ...
            let old;
            try {
                old = JSON.parse(oldBytes.toString('utf8'));
            }
            catch {
                throw new Error(`Invalid migration lock: ${lockPath}`);
            }
            if (!Number.isSafeInteger(old.pid) || old.pid <= 0 || typeof old.token !== 'string')
                throw new Error(`Invalid migration lock: ${lockPath}`);
            let alive = true;
            try {
            ...
            if (alive)
                throw new Error(`Another AI Factory operation holds the project migration lock (pid ${old.pid}). Retry after it finishes.`);
            if (!equalBytes(await readOptional(lockPath), oldBytes))
                throw new Error('Migration lock changed concurrently. Retry.');
    ...
    try {
        ...
        if (equalBytes(await readOptional(lockPath), lockBytes))
            await fs.unlink(lockPath);
        logSkillTarget('lock:released', { project });
    }
}
async function readJournalBlob(directory, filename, digest) {
    if (digest === null)
        return null;
    ...
    if (!(await fs.lstat(file)).isFile() || (await fs.lstat(file)).isSymbolicLink())
        throw new Error(`Unsafe recovery blob: ${file}`);
    const bytes = await fs.readFile(file);
    if (bytesDigest(bytes) !== digest)
        throw new Error(`Recovery blob digest mismatch: ${file}`);
    return bytes;
}
async function verifyJournal(projectDir, root, raw) {
    const journal = raw;
    if (!journal || ![1, 2].includes(journal.version) || !/^[a-f0-9-]{36}$/.test(journal.id)
        || !['prepared', 'writing', 'committed'].includes(journal.phase)
        || !Array.isArray(journal.files) || !Array.isArray(journal.cleanupDirectories))
        throw new Error('Invalid skill migration journal. Preserve recovery material.');
    ...
    if (!after)
        throw new Error('Missing committed config snapshot.');
    const validMode = (mode, digest) => digest === null ? mode === null
    ...
    if (journal.version === 2 && (!validMode(journal.configBeforeMode, journal.configBefore)
        || !validMode(journal.configAfterMode, journal.configAfter)))
        throw new Error('Invalid config permissions in recovery journal.');
    const snapshots = [JSON.parse(after.toString('utf8'))];
    if (before)
        snapshots.push(JSON.parse(before.toString('utf8')));
    ...
    for (const config of snapshots) {
        ...
        for (const group of groups)
            allowedRoots.add(group.physicalPath);
    }
    const seen = new Set();
    const allowedCleanup = new Set();
    for (const [index, file] of journal.files.entries()) {
        if (!file || typeof file.path !== 'string' || typeof file.physical !== 'string')
    ...
    for (const relative of journal.cleanupDirectories) {
        ...
        if (!allowedCleanup.has(actual))
            throw new Error(`Unsafe cleanup directory: ${relative}`);
    }
    return { journal, directory, before, after };
}
async function matchesJournalConfig(file, journal, bytes, committed) {
...
async function recoveryFileState(target, file, journal, committed) {
...
async function finishJournal(projectDir, root, journal, committed) {
    ...
    for (const [index, file] of journal.files.entries()) {
        ...
        if (identity !== file.physical)
            throw new Error(`Recovery target changed: ${file.path}`);
        if (await recoveryFileState(target, file, journal, committed))
            continue;
        const desired = committed ? file.after : file.before;
        ...
        if (bytes === null)
            ...
            await atomicWrite(target, bytes, (committed ? file.afterMode : file.beforeMode));
    }
    if (!await matchesJournalConfig(configPath, journal, committed ? after : before, committed))
        throw new Error(`Config revision or permissions changed; recovery material retained at ${root}.`);
    if (committed) {
...
export async function recoverSkillMigration(projectDir) {
    await withSkillProjectLock(projectDir, async () => {
        ...
        if (!committed && !await matchesJournalConfig(configPath, journal, before, false))
            throw new Error(`Config revision or permissions conflict with pending migration. Recovery material retained at ${root}.`);
...
export async function applySkillMigration(projectDir, plan, options = {}) {
    return withSkillProjectLock(projectDir, async () => {
        const root = await checkedRuntimeRoot(projectDir);
        if (await readOptional(path.join(root, 'active.json')))
            throw new Error('Pending skill migration must be recovered before applying another plan.');
        const configPath = path.join(projectDir, '.ai-factory.json');
        if (!await matchesFile(configPath, plan.configBefore, plan.configBeforeMode))
            throw new Error('Config revision or permissions changed after migration preflight. No installed files were changed.');
        // Preserve raw native ownership and unknown config fields; only skill fields belong to this transaction.
        const rawConfig = (plan.configBefore ? JSON.parse(plan.configBefore.toString('utf8')) : structuredClone(plan.config));
        if (!Array.isArray(rawConfig.agents)) {
            if (plan.config.agents.length !== 1 || rawConfig.agent !== plan.config.agents[0].id)
                throw new Error('Invalid legacy config schema for skill migration.');
        ...
        for (const agent of rawConfig.agents ?? []) {
            ...
            if (next.managedSkills)
                agent.managedSkills = next.managedSkills;
        }
        const after = Buffer.from(`${JSON.stringify(rawConfig, null, 2)}\n`);
        if (!plan.files.length && equalBytes(after, plan.configBefore))
            return plan.config;
        const id = randomUUID();
        const directory = path.join(root, id);
        await fs.mkdir(directory, { mode: 0o700 });
        const journal = { version: 2, id, phase: 'prepared', configBefore: bytesDigest(plan.configBefore),
        ...
        if (plan.configBefore)
            await fs.writeFile(path.join(directory, 'config.before'), plan.configBefore, { flag: 'wx', mode: 0o600 });
        ...
        for (const [index, file] of plan.files.entries()) {
            ...
            if (!provenRoot)
                throw new Error(`Untracked migration path: ${file.path}`);
            ...
            if (physical !== file.physical)
                throw new Error(`Skill physical identity changed after preflight: ${file.path}`);
            if (!isUnderRoot(physical, provenRoot.physical))
                throw new Error(`Skill path escaped its proven root: ${file.path}`);
            if (!await matchesFile(physical, file.before, file.beforeMode))
                throw new Error(`File or permissions changed after preflight: ${file.path}`);
            ...
            if (file.before)
                await fs.writeFile(path.join(directory, `${index}.before`), file.before, { flag: 'wx', mode: 0o600 });
            if (file.after)
                await fs.writeFile(path.join(directory, `${index}.after`), file.after, { flag: 'wx', mode: 0o600 });
        }
        ...
        try {
            ...
            for (const [index, file] of plan.files.entries()) {
                if (!file.after)
                    continue; // Sources survive until the config commit is durable.
                ...
                if (identity !== journal.files[index].physical || !await matchesFile(physical, file.before, file.beforeMode))
                    throw new Error(`Concurrent destination change or permissions change: ${file.path}`);
            ...
            for (const [index, file] of plan.files.entries()) {
                const physical = await physicalProjectPath(projectDir, file.path);
                if (physical !== journal.files[index].physical || !await matchesFile(physical, file.after ?? file.before, file.after !== null ? file.afterMode : file.beforeMode))
                    throw new Error(`Concurrent migration file change or permissions change: ${file.path}`);
            }
            if (!await matchesFile(configPath, plan.configBefore, plan.configBeforeMode))
                throw new Error('Concurrent config revision or permissions change before migration commit.');
            ...
            for (const warning of plan.warnings)
                console.warn(`[skill-migration] ${warning}`);
            ...
            if (journal.phase !== 'committed') {
                try {
                    await finishJournal(projectDir, root, journal, false);
                }
                catch (rollbackError) {
                    throw new Error(`Skill migration failed: ${error.message}. Recovery stopped: ${rollbackError.message}. Backups: ${directory}`);
                }
            }
            throw new Error(`Skill migration ${journal.phase === 'committed' ? 'committed with pending cleanup' : 'rolled back'}: ${error.message}. Recovery material: ${directory}`);
        }
    });
}
export async function prepareSkillTargets(projectDir, config, groups) {
    return withSkillProjectLock(projectDir, async () => {
        ...
        if (pending) {
            ...
            if (recovered)
                Object.assign(config, recovered);
        }
        const resolved = groups ?? await resolveSkillTargets(projectDir, config.agents);
        const previousGroups = await resolveSkillTargets(projectDir, config.agents, { select: false });
        const extensions = await loadAllExtensions(projectDir, (config.extensions ?? []).map(extension => extension.name));
...
export async function captureSharedSkillRollback(projectDir, agents, names, options = {}) {
    const snapshots = [];
    const flatFiles = [];
    for (const group of await resolveSkillTargets(projectDir, agents, { select: false })) {
        if (!options.includeSingletons && (group.targets.length < 2 || !group.targets.every(target => ['codex', 'codex-app'].includes(target.id))))
            continue;
        for (const name of new Set(names.map(name => path.posix.basename(name.replaceAll('\\', '/'))))) {
            if (!name || name === '.' || name === '..')
                throw new Error('Unsafe shared rollback skill name.');
            ...
            const physical = await physicalProjectPath(projectDir, relative);
            if (!snapshots.some(snapshot => snapshot.physical === physical))
                snapshots.push({ relative, physical, tree: await inventory(physical) });
        }
    }
    return async () => {
        for (const file of flatFiles) {
            if (await physicalProjectPath(projectDir, file.relative) !== file.physical)
                throw new Error(`Flat rollback target changed: ${file.relative}`);
            const target = await physicalProjectPath(projectDir, file.relative, { preserveCase: true });
            if (await matchesFile(target, file.before, file.mode))
                continue;
            if (file.before === null)
                await fs.unlink(target).catch(error => { if (error.code !== 'ENOENT')
                    throw error; });
            else
                await atomicWrite(target, file.before, file.mode);
            logSkillTarget('[FIX:155] replacement:restored-flat-snapshot', { target: file.relative });
        }
        for (const snapshot of snapshots) {
            if (await physicalProjectPath(projectDir, snapshot.relative) !== snapshot.physical)
                throw new Error(`Shared rollback target changed: ${snapshot.relative}`);
            const current = await inventory(snapshot.physical);
            for (const [relative, bytes] of snapshot.tree?.files ?? []) {
                if (equalBytes(current?.files.get(relative) ?? null, bytes) && current?.modes.get(relative) === snapshot.tree.modes.get(relative))
                    continue;
            ...
            for (const relative of current?.files.keys() ?? []) {
                if (!snapshot.tree?.files.has(relative))
                    await fs.unlink(path.join(snapshot.physical, relative));
            }
            logSkillTarget('[FIX:155] replacement:restored-snapshot', { target: snapshot.relative, files: snapshot.tree?.files.size ?? 0 });
            for (const relative of snapshot.tree?.directories ?? [])
                await fs.mkdir(path.join(snapshot.physical, relative), { recursive: true });
            for (const relative of [...(current?.directories ?? [])].sort((a, b) => b.length - a.length)) {
                if (!snapshot.tree?.directories.includes(relative))
                    await fs.rmdir(path.join(snapshot.physical, relative)).catch(error => {
                        if (!['ENOENT', 'ENOTEMPTY', 'EEXIST'].includes(error.code))
                            throw error;
                    });
            }
...
//# sourceMappingURL=skills-migration.js.map