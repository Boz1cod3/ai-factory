...
async function extensionAddLocked(source) {
...
async function extensionRemoveLocked(name) {
    const projectDir = process.cwd();
...
export async function extensionUpdateCommand(name, options) {
    return withSkillProjectLock(process.cwd(), () => extensionUpdateLocked(name, options));
}
async function extensionUpdateLocked(name, options) {
    const projectDir = process.cwd();
