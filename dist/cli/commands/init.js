...
import { buildBundledAgentFileSources, buildExtensionAgentFileSources, buildManagedConfigFilesState, buildManagedSkillsState, installConfigFiles, installSkills, installSubagents, getAvailableSkills, rebuildManagedAgentFilesForAgents, resolveManagedConfigFilePaths, resolveInstalledAgentFileTargetPath, removeOwnedSkills, } from '../../core/installer.js';
...
import { assertNoAgentFileConflicts, collectReplacedSkills, installExtensionAgentFilesForAllAgents, mergeAgentFileSources, mergeInstalledAgentFiles, composeInstalledExtensionSkills, } from '../../core/extension-ops.js';
...
async function removeAgentSetup(projectDir, agent, installedExtensionManifests = [], survivingAgents = []) {
    ...
    if (agentsDir) {
        ...
        for (const relPath of managedFiles) {
            try {
                ...
                console.log(chalk.yellow(`Warning: Skipping unsafe managed agent file path "${relPath}" while removing ${agent.id}: ${error.message}`));
            }
        }
    }
    ...
    for (const relPath of configFiles) {
        try {
            ...
            console.log(chalk.yellow(`Warning: Skipping unsafe managed config file path "${relPath}" while removing ${agent.id}: ${error.message}`));
        }
    }
...
async function initLocked(options) {
    ...
    try {
        ...
        const groups = await resolveSkillTargets(projectDir, answers.agents.map(agent => ({
            id: agent.id,
            skillsDir: existingConfig?.agents.find(previous => previous.id === agent.id)?.skillsDir ?? getAgentConfig(agent.id).skillsDir,
        })));
        if (existingConfig)
            await prepareSkillTargets(projectDir, existingConfig, groups);
        ...
        const survivingAgents = answers.agents.map(selection => ({
        ...
        for (const agent of installedAgents) {
            ...
            agent.managedSkills = await buildManagedSkillsState(projectDir, agent, managedBaseSkills, groups.find(group => group.targets.some(target => target.id === agent.id)).context, existingExtensions);
        ...
        if (nonInteractive && (message.startsWith('Unknown ')
