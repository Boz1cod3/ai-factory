        ...
        for (const { dir, manifest } of extensions) {
            ...
            for (const cmd of manifest.commands) {
                try {
                    ...
                    console.error(`Warning: Failed to load command "${cmd.name}" from extension "${manifest.name}": ${err.message}`);
                }
            }
        }
    }
    catch (err) {
        console.error(`Warning: Failed to load extension commands: ${err.message}`);
    }
}
